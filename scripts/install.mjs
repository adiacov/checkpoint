...
function isRepoSymlink(target) {
	...
	return real != null && (real === REPO_ROOT || real.startsWith(`${REPO_ROOT}/`));
}
...
function readManifest(opts) {
	...
	if (!obj || !Array.isArray(obj.entries)) return { version: MANIFEST_VERSION, entries: [] };
	return obj;
}
...
function record(manifest, entry) {
	...
	const full = { marker: null, installedAt: new Date().toISOString(), ...entry };
...
function findEntry(manifest, target, marker = null) {
	return manifest.entries.find((e) => e.target === target && (e.marker ?? null) === marker);
}
...
function planBuild(agent, adapterDir, opts) {
	if (opts.build === false) {
		if (!distExists(adapterDir)) {
			throw new Error(`dist/ missing for ${agent}; build it or drop --no-build`);
		}
		return null; // --no-build trusts an existing dist (no staleness check)
	}
	if (!isStale(adapterDir)) return null;
	return action(agent, join(adapterDir, "dist"), "build", "build", "dist stale → npm run build", () =>
		runBuild(adapterDir),
	);
}
...
function planPlacement(agent, source, target, opts, manifest) {
	...
	const apply = () => {
		...
		record(manifest, { agent, type: mode === "symlink" ? "link" : "copy", target, mode });
	};
	...
	if (!ours) {
		if (!opts.force) {
			return action(agent, target, mode, "conflict", "exists, not installed by checkpoint; --force to replace");
		}
		return action(agent, target, mode, "updated", "replaced user content (--force)", apply);
	}

	// Tool-managed already — is it already in the desired state for this mode?
	if (mode === "symlink") {
		if (st && st.isSymbolicLink() && realpathSafe(target) === realpathSafe(source)) {
...
function planUninstallPlacement(agent, target, manifest) {
	const st = lstatSafe(target);
	const entry = findEntry(manifest, target);
	if (!st && !entry) return action(agent, target, "-", "no-op", "nothing to remove");
	const ours = isRepoSymlink(target) || !!entry;
	if (!ours) return action(agent, target, "-", "no-op", "present but not installed by checkpoint; left intact");
	return action(agent, target, entry?.mode ?? "symlink", "removed", "removed", () => {
		removePath(target);
...
function findManagedNotify(lines) {
	...
	return { sentinelIdx: idx, notifyIdx: idx + 1, current: lines[idx + 1].trim() };
}
...
function hasUserRootNotify(lines, managed) {
	for (let i = 0; i < lines.length; i++) {
		...
		if (managed && (i === managed.sentinelIdx || i === managed.notifyIdx)) continue;
...
function planCodexNotify(ctx) {
	...
	const managed = findManagedNotify(lines);
	if (managed) {
		if (managed.current === desired) return action("codex", cfg, "config", "no-op", "notify up to date");
		return action("codex", cfg, "config", "updated", "updated notify path", () => {
			writeFileSync(cfg, applyCodexNotify(readFileSafe(cfg) ?? "", desired, { force: ctx.opts.force }));
			record(ctx.manifest, { agent: "codex", type: "config", target: cfg, marker: CODEX_SENTINEL });
		});
	}
	if (hasUserRootNotify(lines, null) >= 0 && !ctx.opts.force) {
		return action("codex", cfg, "config", "conflict", "existing notify present; --force to override");
	}
...
function planUninstallCodexNotify(ctx) {
	...
	return action("codex", cfg, "config", "removed", "removed managed notify", () => {
...
const HANDLERS = {
	pi: {
		...
		planInstall(ctx) {
			...
			if (lstatSafe(legacy)) {
				if (ctx.opts.force) {
					acts.push(action("pi", legacy, "-", "removed", "removed legacy reference (--force)", () => removePath(legacy)));
				} else {
					acts.push(action("pi", legacy, "-", "conflict", "legacy reference present; --force to replace"));
				}
			}
			return acts;
		},
		planUninstall(ctx) {
			return [planUninstallPlacement("pi", join(ctx.roots.pi, "checkpoint"), ctx.manifest)];
		},
	},
	...
	codex: {
		...
		planInstall(ctx) {
			...
			for (const name of readdirSync(promptsDir)) {
				acts.push(
					planPlacement("codex", join(promptsDir, name), join(ctx.roots.codex, "prompts", name), ctx.opts, ctx.manifest),
				);
			}
		...
		planUninstall(ctx) {
			...
			const promptsDir = join(this.dir, "prompts");
			for (const name of readdirSync(promptsDir)) {
				acts.push(planUninstallPlacement("codex", join(ctx.roots.codex, "prompts", name), ctx.manifest));
			}
...
export async function run(opts) {
	...
	for (const agent of opts.agents) {
		try {
			...
			else if (opts.verb === "uninstall") actions.push(...handler.planUninstall(ctx));
	...
	if (opts.verb !== "status" && !opts.dryRun) {
		...
		for (const a of actions) {
			...
			if (a.apply && ["installed", "updated", "removed", "build"].includes(a.action)) {
				try {
					await a.apply();
				} catch (e) {
					a.action = "failed";
...
function statusActions(agent, ctx) {
	...
	if (mine.length === 0) return [action(agent, "-", "-", "no-op", "not installed")];
	return mine.map((e) => action(agent, e.target, e.mode ?? e.type, "no-op", `installed (${e.type})`));
}

function exitCodeFor(actions) {
	return actions.some((a) => a.action === "conflict" || a.action === "failed") ? 1 : 0;
}
...
export function formatReport(opts, actions) {
	...
	lines.push("");
	for (const a of actions) {
		lines.push(`  ${a.agent.padEnd(7)} ${String(a.target).padEnd(48)} ${String(a.mode).padEnd(8)} ${a.action.padEnd(10)} ${a.detail ?? ""}`.trimEnd());
	}
	lines.push("");
	const counts = {};
	for (const a of actions) counts[a.action] = (counts[a.action] ?? 0) + 1;
	const order = ["installed", "updated", "removed", "no-op", "skipped", "conflict", "failed", "build"];
	lines.push(`summary: ${order.filter((k) => counts[k]).map((k) => `${counts[k]} ${k}`).join(", ") || "nothing to do"}`);
	return lines.join("\n");
}
...
export function parseArgs(argv) {
	...
	if (!["install", "uninstall", "status"].includes(opts.verb)) {
		throw new UsageError(`expected verb install|uninstall|status, got: ${opts.verb ?? "(none)"}`);
	}
	if (!["symlink", "copy"].includes(opts.mode)) throw new UsageError(`bad --mode: ${opts.mode}`);
	if (agentSel.length) {
		const sel = agentSel.includes("all") ? AGENTS.slice() : agentSel;
		for (const a of sel) if (!AGENTS.includes(a)) throw new UsageError(`unknown agent: ${a}`);
...
if (process.argv[1] && realpathSafe(process.argv[1]) === fileURLToPath(import.meta.url)) {
	main();
}
